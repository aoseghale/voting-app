...
class EditCommentTest extends TestCase
{
    ...
    public function edit_comment_form_validation_works()
    {
        ...
        Livewire::actingAs($user)
            ...
            ->call('updateComment')
            ->assertHasErrors(['body'])
            ->assertSee('The body must be at least 4 characters');
    }
    ...
    public function editing_a_comment_works_when_user_has_authorization()
    {
        ...
        Livewire::actingAs($user)
            ->test(EditComment::class)
            ->call('setEditComment', $comment->id)
            ->set('body', 'This is my edited comment')
            ->call('updateComment')
    ...
    /** @test */
    public function editing_a_comment_does_not_work_when_user_does_not_have_authorization_because_different_user_created_comment()
    {
        $user = User::factory()->create();
    ...
    /** @test */
    public function edit_comment_shows_on_menu_when_user_has_authorization()
    {
        $user = User::factory()->create();
        $idea = Idea::factory()->create();
    ...
    /** @test */
    public function edit_comment_does_not_show_on_menu_when_user_does_not_have_authorization()
    {
        $user = User::factory()->create();
        $idea = Idea::factory()->create();
